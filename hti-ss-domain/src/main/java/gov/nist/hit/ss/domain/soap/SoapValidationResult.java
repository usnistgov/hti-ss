 ...
 */
package gov.nist.hit.ss.domain.soap;
...
import gov.nist.hit.ss.domain.MessageFailureV3;
import gov.nist.hit.ss.domain.jaxb.HL7V3MessageReport;
import gov.nist.hit.ss.domain.jaxb.HL7V3MessageReport.AssertionList;
import gov.nist.hit.ss.domain.jaxb.HL7V3MessageReport.AssertionList.Assertion;
import gov.nist.hit.ss.domain.jaxb.HL7V3MessageReport.AssertionList.Assertion.Location;
import gov.nist.hit.ss.domain.jaxb.HL7V3MessageReport.MetaData;
import gov.nist.hit.ss.domain.jaxb.HL7V3MessageReport.MetaData.Message;
import gov.nist.hit.ss.domain.jaxb.HL7V3MessageValidationReport;
import gov.nist.hit.ss.domain.jaxb.ReportHeader.TestObjectReferenceList;
import gov.nist.hit.ss.domain.jaxb.ReportHeader.TestObjectReferenceList.TestObjectReference;
import gov.nist.hit.ss.domain.jaxb.TestObjectReferenceType;
...
public class SoapValidationResult {
	 ...
	 */
	public gov.nist.hit.ss.domain.jaxb.HL7V3MessageValidationReport createReport()
	 ...
	 * @throws SOAPException
	 */
	public gov.nist.hit.ss.domain.jaxb.HL7V3MessageValidationReport createReport(String messageFilename)
			throws JAXBException, DatatypeConfigurationException, SOAPException, IOException {
		report = new gov.nist.hit.ss.domain.jaxb.HL7V3MessageValidationReport();
		setReportHeader(report);
		setReportSpecific(report, messageFilename);
		return report;
	}

	private void setReportHeader(gov.nist.hit.ss.domain.jaxb.HL7V3MessageValidationReport report)
			throws DatatypeConfigurationException, SOAPException, IOException {
		gov.nist.hit.ss.domain.jaxb.ReportHeader header = new gov.nist.hit.ss.domain.jaxb.ReportHeader();
		report.setHeaderReport(header);
		header.setValidationStatus(gov.nist.hit.ss.domain.jaxb.ValidationStatusType.COMPLETE);
		...
		header.setStandardType(gov.nist.hit.ss.domain.jaxb.StandardTypeType.CDC_WSDL);
		header.setValidationType(gov.nist.hit.ss.domain.jaxb.ValidationType.AUTOMATED);
		// Date Test
	...
	private void setReportSpecific(gov.nist.hit.ss.domain.jaxb.HL7V3MessageValidationReport report,
			String messageFilename) {
		...
		while (itMF.hasNext()) {
			...
			assertion.setType(gov.nist.hit.ss.domain.jaxb.AssertionTypeV3Constants.valueOf(mf.getFailureType()));
			assertion.setResult(gov.nist.hit.ss.domain.jaxb.AssertionResultConstants.valueOf(mf.getAssertionResult()));
			assertion.setSeverity(gov.nist.hit.ss.domain.jaxb.ErrorSeverityConstants.valueOf(mf.getFailureSeverity()));
	...
	protected void count() {
		...
		for (MessageFailureV3 mf : messageFailures) {
			...
			if (assertionResult.equals(gov.nist.hit.ss.domain.jaxb.AssertionResultConstants.ERROR.toString())) {
				errorCount++;
			} else if (assertionResult
					.equals(gov.nist.hit.ss.domain.jaxb.AssertionResultConstants.WARNING.toString())) {
				warningCount++;
			} else if (assertionResult.equals(gov.nist.hit.ss.domain.jaxb.AssertionResultConstants.IGNORE.toString())) {
				ignoreCount++;
			} else if (assertionResult.equals(gov.nist.hit.ss.domain.jaxb.AssertionResultConstants.ALERT.toString())) {
				alertCount++;
			} else if (assertionResult
					.equals(gov.nist.hit.ss.domain.jaxb.AssertionResultConstants.AFFIRMATIVE.toString())) {
				affirmativeCount++;
			} else if (assertionResult
					.equals(gov.nist.hit.ss.domain.jaxb.AssertionResultConstants.INFORMATIONAL.toString())) {
				informationalCount++;
			} else {
				errorCount++;
			}
	...
	public String toXml() throws DatatypeConfigurationException, SOAPException, IOException {
		...
		try {
			...
			gov.nist.hit.ss.domain.jaxb.ReportHeader header = report.getHeaderReport();
