package gov.nist.hit.ss.service.soap;
...
import gov.nist.hit.ss.domain.soap.SoapMessage;
import gov.nist.hit.ss.domain.soap.SoapValidationResult;
...
public class CDCSoapValidation {
	...
	private void setErrLineNumbers(String soapXml, ArrayList<gov.nist.hit.ss.domain.MessageFailureV3> soapFailures) {
		// the SAXBuilder is the easiest way to create the JDOM2 objects.
		SAXBuilder jdomBuilder = new SAXBuilder();
		jdomBuilder.setJDOMFactory(new LocatedJDOMFactory());
		Iterator<gov.nist.hit.ss.domain.MessageFailureV3> iterator = soapFailures.iterator();
		while (iterator.hasNext()) {
			gov.nist.hit.ss.domain.MessageFailureV3 failure = iterator.next();
